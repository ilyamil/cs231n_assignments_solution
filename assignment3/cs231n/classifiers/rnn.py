...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if (self.cell_type == 'rnn'):
          hidden_states, cache = rnn_forward(word_emb, h_init, Wx, Wh, b)
        if (self.cell_type == 'lstm'):
          hidden_states, cache = lstm_forward(word_emb, h_init, Wx, Wh, b)
        # 4 forward
        ...
        # 4 backward
        dx, dW_vocab, db_vocab = temporal_affine_backward(dx, cache_affine)
        # 3 backward
        if (self.cell_type == 'rnn'):
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
          if (self.cell_type == 'rnn'):
            next_h, _ = rnn_step_forward(x, h_init, Wx, Wh, b)
          if (self.cell_type == 'lstm'):
            next_h, next_c, _ = lstm_step_forward(x, h_init, c_init, Wx, Wh, b)

          scores = np.dot(next_h, W_vocab) + b_vocab
          highest_score_idx = np.argmax(scores, axis=1)
          captions[:,t] = highest_score_idx
          x = W_embed[highest_score_idx]
          h_init = next_h
          c_init = next_c
